...
def return_description_properties(properties_path_excel, flow_curve_excel_path):
    ...
    mechanical_values_list = properties_df["mechanical_normalized_values"].dropna().tolist()

    diffusion_descriptions_list = properties_df["diffusion_descriptions"].dropna().tolist()
    diffusion_values_list = properties_df["diffusion_normalized_values"].dropna().tolist()
    ...
    description_properties_dict["diffusion_properties"] = dict(zip(diffusion_descriptions_list, diffusion_values_list))
...
def return_UMAT_property(description_properties_dict): 
    ...
    for line_index in range(mech_prop_num_lines):
        if line_index != mech_prop_num_lines - 1:
            ...
            UMAT_property.append("** " + ", ".join(subset_description[0:8]))
...
def return_UMATHT_property(description_properties_dict): 
    ...
    diffusion_properties_list = list(description_properties_dict["diffusion_properties"].values())
    diffusion_description_list = list(description_properties_dict["diffusion_properties"].keys())

    # Abaqus needs to define 8 properties each line
    diffusion_prop_num_lines = int(np.ceil(len(diffusion_properties_list)/8))
    diffusion_prop_num_properties = int(diffusion_prop_num_lines*8)

    total_UMATHT_num_properties = diffusion_prop_num_properties 
    ...
    for line_index in range(diffusion_prop_num_lines):
        if line_index != diffusion_prop_num_lines - 1:
            subset_properties = diffusion_properties_list[line_index*8:(line_index+1)*8]
            subset_description = diffusion_description_list[line_index*8:(line_index+1)*8]
            UMATHT_property.append("** " + ", ".join(subset_description[0:8]))
            UMATHT_property.append(", ".join(subset_properties))
            # UMATHT_property.append("** " + ", ".join(subset_description[4:8]))
        else:
            subset_properties = diffusion_properties_list[line_index*8:] + ["0.0"]*(8-len(diffusion_properties_list[line_index*8:]))
            subset_description = diffusion_description_list[line_index*8:] + ["none"]*(8-len(diffusion_description_list[line_index*8:]))
            UMATHT_property.append("** " + ", ".join(subset_description[0:8]))
            UMATHT_property.append(", ".join(subset_properties))
            # UMATHT_property.append("** " + ", ".join(subset_description[4:8]))
...
def return_depvar(depvar_excel_path):
    ...
    depvar_initial_values = depvar_df["depvar_initial_values"].tolist()
    ...
    return DEPVAR, nstatev, depvar_initial_values
...
def main():
    ...
    #########################################
    # STEP 4: Replacing the density as RHO0 #
    #########################################

    # Note: normalized <RHO0> is a key in the mechanical properties
    # Find the index of the *Density section
    density_index = [i for i, line in enumerate(flines) if '*DENSITY' in line.upper()][0]

    RHOD0_normalized_value = description_properties_dict["mechanical_properties"]["<RHO0>"]

    # The line below it is to be replaced
    flines[density_index + 1] = f"{RHOD0_normalized_value},"

    #########################################
    # STEP 5: Extracting the DEPVAR section #
    #########################################
    ...
    DEPVAR, nstatev, depvar_initial_values = return_depvar(depvar_excel_path)
    
    flines = flines[:depvar_index] + DEPVAR + flines[depvar_index+2:]


    #################################################
    # STEP 6: Writing the initial values for DEPVAR #
    #################################################

    INITIAL_CONDITIONS_SOLUTION = [
        "**",
        "******** INITIAL CONDITIONS ********",
        "**",
    ...
    # First we postprocess the depvar_initial_values
    depvar_initial_values_filtered = []

    for value in depvar_initial_values:
        try:
            # Try converting the value to float, meaning it's already a number
            depvar_initial_values_filtered.append(float(value))
        except ValueError:
            # If conversion fails, assume it's a placeholder key (like <RHO0>)
            if value in description_properties_dict["mechanical_properties"]:
                depvar_initial_values_filtered.append(description_properties_dict["mechanical_properties"][value])
            else:
                raise KeyError(f"Key {value} not found in mechanical properties.")


    # Format the values into rows of 8 (Abaqus style)
    if nstatev < 8:
        INITIAL_CONDITIONS_SOLUTION.append("ALLE, " + ", ".join(map(str, depvar_initial_values_filtered)))
    else:
        # First line with 7 values (Abaqus requires this formatting)
        INITIAL_CONDITIONS_SOLUTION.append("ALLE, " + ", ".join(map(str, depvar_initial_values_filtered[:7])))

        # Append full rows of 8 values
        for i in range(7, nstatev - 8, 8):  # Start from 7, step by 8
            INITIAL_CONDITIONS_SOLUTION.append(", ".join(map(str, depvar_initial_values_filtered[i:i+8])))

        # Append the remaining values if not a multiple of 8
        remaining_values = depvar_initial_values_filtered[(nstatev//8) * 8:]
        if remaining_values:
            INITIAL_CONDITIONS_SOLUTION.append(", ".join(map(str, remaining_values)))
    
    INITIAL_CONDITIONS_SOLUTION.append("******************************")

    boundary_conditions_index = [i for i, line in enumerate(flines) if '** BOUNDARY CONDITIONS' in line.upper()][0]
    initial_conditions_index = boundary_conditions_index - 2

    flines = flines[:initial_conditions_index] + INITIAL_CONDITIONS_SOLUTION + flines[initial_conditions_index+1:]
    ...
    # We have to add it two lines after the loine that starts with *FIELD, VARIABLE
    ...
    flines = flines[:control_temperature_index] + control_temperature + flines[control_temperature_index:]

    # #########################################
    # # STEP 8: Changing the output variables #
    # #########################################
